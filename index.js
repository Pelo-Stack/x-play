...
const { help,info, donasi, readme }= require("./lib/help.js");
...
axios.get(`https://alfians-api.herokuapp.com/api/yta?url=${teks}`).then((res) => {
    ...
    let hasil = `鉁匧agu Berhasil Di Download, silahkan klik link dan download hasilnya\nKlik link dibawah馃棥锔廫n\nJudul: ${res.data.title}\n\nUkuran audio: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
      break
case 'igstalk:'
  const teks = text.replace(/igstalk /, "")
  axios.get(`https://alfians-api.herokuapp.com/api/stalk?username=${teks}`).then ((res) =>{
  conn.sendMessage(id, '[WAIT] Stalking...鈴�', MessageType.text)
  let hasil = `鉁˙iodata Instagram _${teks}_ \n\n 馃Ф *Username* : _${res.data.Username}_ \n 馃寑 *Nama* : _${res.data.Name}_ \n 馃専 *Jumlah Followers* : _${res.data.Jumlah_Followers}_ \n 馃尃 *Jumlah_Following* : _${res.data.Jumlah_Following}_ \n 猸� *Jumlah_Post* : _${res.data.Jumlah_Post}_ `;
  conn.sendMessage(id, hasil, MessageType.text);
     break
case 'ytmp4':
...
axios.get(`https://alfians-api.herokuapp.com/api/ytv?url=${teks}`).then((res) => {
	conn.sendMessage(id, '[ WAIT ] Sedang di proses鈴� silahkan tunggu sebentar', MessageType.text)
    let hasil = `鉁匳ideo Berhasil Di Download, silahkan klik link dan download hasilnya\nKlik link dibawah馃棥锔廫n\nJudul: ${res.data.title}\n\nUkuran video: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
    break
case 'twt':
...
axios.get(`https://mhankbarbar.herokuapp.com/api/twit?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
	conn.sendMessage(id, '[ WAIT ] Sedang di proses鈴� silahkan tunggu sebentar', MessageType.text)
    let hasil = `鉁匓erhasil! silahkan klik link di bawah untuk mendownload hasilnya!\nKlik link dibawah馃棥锔廫n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
         break
case 'tiktok':
...
axios.get(`https://st4rz.herokuapp.com/api/tiktok?url=${tictoc}`).then((res) => {
	 conn.sendMessage(id, '[ WAIT ] Sedang di proses鈴� silahkan tunggu sebentar', MessageType.text)
     let titoe = `鉁匓erhasil!!! Silahkan klik link dibawah ini untuk mendownload hasilnya! \nKlik link dibawah馃棥锔廫n\nJudul: ${res.data.deskripsi} \n\nDurasi: ${res.data.durasi}\n\nNama: ${res.data.nama}\n\nUrl: ${res.data.urlvideo}`;
...
case 'sholat':
  ...
  conn.sendMessage(id, '[ WAIT ] Sedang di proses鈴� silahkan tunggu sebentar', MessageType.text)
  let hasil = `Jadwal sholat di ${teks} hari ini adalah\n\n鈿msyak : ${res.data.Imsyak}\n鈿ubuh : ${res.data.Subuh} WIB\n鈿zuhur : ${res.data.Dzuhur}WIB\n鈿shar : ${res.data.Ashar} WIB\n鈿aghrib : ${res.data.Maghrib}\n鈿sya : ${res.data.Isya} WIB\n鈿engah malam : ${res.data.Dhuha} WIB`;
  conn.sendMessage(id, hasil, MessageType.text);
            break
            case 'pict':
           switch (value) {
               case 'cewek':
                   cewePict(value)
                       .then(buffer => {
                           client.sendMessage(id, '[鉂梋 WAIT BOSQ馃枻',MessageType.text)
...
const get = require('got')
    ...
    conn.sendMessage(id,`馃搶DATA WABAH COVID-19 TERBARU DI INDONESIA\n\n馃搷Positif ==> ${positif} \n馃搷Sembuh ==> ${sembuh} \n馃搷Meninggal ==> ${meninggal}\n馃搷Dirawat ==> ${dirawat}`, MessageType.text);
           break
           case 'lirik':
	const teks = text.split("lirik")[1]
	axios.get(`http://scrap.terhambar.com/lirik?word=${teks}`).then ((res) => {
	     conn.sendMessage(id, '[ WAIT ] Sedang di proses鈴� silahkan tunggu sebentar', MessageType.text)
	 	let hasil = `馃搷lirik lagu馃搷${teks} \n\n\n ${res.data.result.lirik}`
	conn.sendMessage(id, hasil, MessageType.text)
	   break
case 'alay':
